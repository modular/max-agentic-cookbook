...
class ChatHandler(BaseHandler):
    ...
    async def handle(self, context: QueryContext):
        ...
        if context.repo_url and (not self.current_context or self.current_context != context.repo_url):
...
Keep responses concise but informative, and focus on practical, actionable advice."""
        ...
        response_panel = Panel("", title="[bold green]Assistant[/bold green]", border_style="green")
...
class RepoQAHandler(BaseHandler):
    ...
    async def handle(self, context: QueryContext):
        ...
        if context.repo_url and context.repo_url != self.current_repo_url:
            ...
            self.collection_name = f"{repo_info['user']}_{repo_info['repo']}_chunks".lower().replace('-', '_')
        ...
        if context.repo_url and not context.repo_content:
            with self.console.status("[bold green]Processing repository...", spinner="dots"):
                ...
                await code_parse(context.repo_content, self.collection_name, console=self.console, use_progress=False)
        ...
        if context.repo_content and not self.embeddings_created and self.collection_name:
            with self.console.status("[bold green]Creating embeddings for cached content...", spinner="dots"):
                await code_parse(context.repo_content, self.collection_name, console=self.console, use_progress=False)
                self.embeddings_created = True
                self.console.print("[bold green]Embeddings created for cached repository content[/bold green]")
        ...
        if self.collection_name:
            try:
                with self.console.status("[bold green]Searching for relevant code...", spinner="dots"):
                    search_results = await semantic_search(context.query, self.collection_name, top_k=3)
                    ...
                    if search_results:
                        ...
                        self.console.print(f"[bold green]Found {len(relevant_chunks)} relevant code chunks[/bold green]")
            except Exception as e:
                self.console.print(f"[bold yellow]Search error: {e}[/bold yellow]")
                self.console.print("[bold yellow]Continuing without semantic search results[/bold yellow]")
...
Be specific about file paths and line numbers when referring to code."""
        ...
        full_response = ""
        response_panel = Panel("", title="[bold green]Repository Expert[/bold green]", border_style="green")
...
class DocGenHandler(BaseHandler):
    async def handle(self, context: QueryContext) -> str:
        ...
        with self.console.status("[bold green]Ingesting repository...", spinner="dots"):
            ...
            self.console.print(f"[bold green]Repository ingested in {elapsed:.2f} seconds[/bold green]")
        ...
        for readme_file in readme_files:
            if readme_file in content:
                ...
                self.console.print(f"[bold green]Found README: {readme_file}[/bold green]")
                break
        ...
        table.add_row("Total size", f"{sum(len(c) for c in content.values()) / 1024:.2f} KB")
        ...
        self.console.print(f"[bold green]Code parsed: {len(chunks)} chunks identified[/bold green]")
        ...
        with Progress(
            ...
            doc_task = progress.add_task("[bold blue]Generating documentation...", total=len(chunks))
    ...
    async def _process_doc_batch(self, batch: List[CodeChunk], language: str) -> Dict[str, str]:
    ...
    async def _generate_examples(self, documentation: Dict[str, str], primary_language: str) -> Dict[str, str]:
        ...
        with Progress(
            ...
            example_task = progress.add_task("[bold blue]Generating examples...", total=3)
            ...
            response = await self.client.chat.completions.create(
                model=LLM_MODEL,
                messages=[
                    {"role": "system", "content": "You are an expert at categorizing code components."},
            ...
            example_prompts = {
                'basic': self._create_example_prompt(categories['basic'], documentation, primary_language, 'basic'),
                'intermediate': self._create_example_prompt(categories['intermediate'], documentation, primary_language, 'intermediate'),
                'advanced': self._create_example_prompt(categories['advanced'], documentation, primary_language, 'advanced')
            }
            ...
            for level, prompt in example_prompts.items():
                tasks.append(self.client.chat.completions.create(
                    model=LLM_MODEL,
                    messages=[
                        {"role": "system", "content": """You are an expert developer creating educational code examples..."""},
            ...
            return {
                ...
                'intermediate': clean_code_example(results[1].choices[0].message.content),
                'advanced': clean_code_example(results[2].choices[0].message.content)
            }

    def _create_example_prompt(self, components: List[str], documentation: Dict[str, str], language: str, level: str) -> str:
...
async def determine_query_type(query: str, client) -> Tuple[QueryType, Optional[str]]:
    ...
    response = await client.chat.completions.create(
        model=LLM_MODEL,
        messages=[
            {"role": "system", "content": "You are an expert at analyzing queries and extracting GitHub URLs."},
...
def should_skip_file(file_path: str) -> bool:
    ...
    skip_paths = [
        '.github/', 'tests/', 'test/', 'examples/', 'demos/',
        'docs/', 'scripts/', 'tools/', '.vscode/', '.idea/',
        '__pycache__/', 'node_modules/', 'dist/', 'build/',
        'venv/', 'env/'
    ]

    skip_extensions = [
        '.md', '.rst', '.txt', '.yml', '.yaml', '.json',
        '.ini', '.cfg', '.toml', '.bat', '.sh', '.ps1',
        '.pyc', '.pyo', '.pyd', '.so', '.dll', '.dylib',
        '.egg-info', '.coverage', '.DS_Store', 'Dockerfile',
        '.dockerignore', '.gitignore', 'LICENSE', 'MANIFEST.in',
    ...
    return (
        any(skip_path in file_path for skip_path in skip_paths) or
        any(file_path.endswith(ext) for ext in skip_extensions) or
        any(re.match(pattern, file_path) for pattern in skip_patterns)
    )
...
async def semantic_search(query: str, collection_name: str, top_k: int = 5) -> List[Dict]:
...
def ensure_code_blocks_closed(content):
    ...
    for line in lines:
        ...
        elif in_code_block and line.strip().startswith('#') and not line.strip().startswith('#!'):
            result_lines.append(line)
        else:
            result_lines.append(line)
...
async def code_parse(content: Dict[str, str], collection_name: str = "code_chunks", console: Optional[Console] = None, use_progress: bool = True) -> List[CodeChunk]:
            ...
            try:
                ...
                for chunk_type, pattern in patterns.items():
                    ...
                    for match in matches:
                        ...
                        name = name_match.group(2) if name_match else f"unnamed_{len(chunks)}"
    ...
    async def generate_embeddings():
        ...
        for i in range(0, len(chunks), BATCH_SIZE):
            ...
            texts = [f"{chunk.file_path}:{chunk.metadata['name']}\n{chunk.content}" for chunk in batch]
    ...
    if use_progress:
        with Progress(
            ...
            parse_task = progress.add_task("[bold blue]Parsing code files...", total=len(content))
            for file_path, code in content.items():
                if should_skip_file(file_path):
            ...
            if chunks:
                embed_task = progress.add_task("[bold blue]Generating embeddings...", total=len(chunks))
                ...
                console.print(f"[bold green]Embeddings generated and stored for {len(chunks)} code chunks[/bold green]")
    else:
        await process_files()
        await generate_embeddings()
        console.print(f"[bold green]Processed {len(content)} files and generated embeddings for {len(chunks)} code chunks[/bold green]")
...
async def verify_code_example(client: AsyncOpenAI, code: str, api_docs: str, name: str) -> str:
...
async def verify_home_content(client: AsyncOpenAI, content: str, documentation: Dict[str, str]) -> str:
...
async def build_doc_site(
    ...
    with Progress(
        ...
        verify_task = progress.add_task(
            ...
            total=1 + len(top_level_examples) + len([doc for doc in documentation.values() if '```python' in doc])
        )
...
"""
        ...
        try:
            ...
            file_tasks = [
                ...
                write_file(os.path.join(docs_dir, "installation.md"), installation_content),

                *[write_file(
                    os.path.join(api_dir, f"{re.sub(r'[^a-zA-Z0-9_-]', '_', name)}.md"),
...
"""),
                ...
                write_file(os.path.join(output_dir, "requirements.txt"), """mkdocs>=1.4.0
mkdocs-material>=9.0.0
pymdown-extensions>=9.0
pygments>=2.14.0
"""),

                write_file(os.path.join(output_dir, "README.md"), f"""# {repo_info['repo']} Documentation
...
"""
            for group, items in components.items():
                if items:
                    api_index_content += f"\n### {group}\n\n"
                    for name in sorted(items):
                        api_index_content += f"- [`{name}`]({re.sub(r'[^a-zA-Z0-9_-]', '_', name)}.md)\n"

            file_tasks.append(write_file(os.path.join(api_dir, "index.md"), api_index_content))

            await asyncio.gather(*file_tasks)

            console.print(f"[bold green]Documentation generated successfully in {output_dir}/[/bold green]")
            console.print(f"[bold green]To view the documentation:[/bold green]")
            console.print(f"[green]1. cd {output_dir}[/green]")
            console.print(f"[green]2. pip install -r requirements.txt[/green]")
            console.print(f"[green]3. mkdocs serve[/green]")
            console.print(f"[green]4. Open http://127.0.0.1:8000/ in your browser[/green]")

        except Exception as e:
            console.print(f"[bold red]Error writing documentation files: {str(e)}[/bold red]")
            raise
...
async def main():
    ...
    while True:
        ...
        if not repo_url and current_repo_url and query_type in [QueryType.DOC_GEN, QueryType.REPO_QA]:
            repo_url = current_repo_url

        context = QueryContext(query=query, repo_url=repo_url, repo_content=repo_content)

        if current_handler_type != query_type:
            if query_type == QueryType.CHAT:
                handlers[QueryType.CHAT].conversation_history = conversation_history.copy()
            elif query_type == QueryType.REPO_QA:
                if repo_url == current_repo_url:
                    handlers[QueryType.REPO_QA].conversation_history = conversation_history.copy()
        ...
        if query_type in [QueryType.DOC_GEN, QueryType.REPO_QA] and not context.repo_url:
            console.print("[bold yellow]Please provide a GitHub repository URL:[/bold yellow] ", end="")
        ...
        if query_type == QueryType.REPO_QA and context.repo_url:
            ...
            if hasattr(handler, '_load_repo_content'):
                if repo_content:
                    ...
                    if existing_content:
                        console.print("[bold yellow]I have cached content for this repository. Use it? (y/n)[/bold yellow] ", end="")
