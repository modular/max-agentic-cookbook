...
- Dropzone for `.jsonl` file upload with client-side parsing and 10MB size validation
...
- SWR mutation (`useSWRMutation`) for batch API request with loading state management
- Results display with performance metrics
- Download functionality to export classified results as JSONL

**Backend** (`batch_text_classification.py`): FastAPI with parallel processing and rate limiting

- Flexible schema support: extracts text from any JSON field
- Parallel processing using `asyncio.gather()` with semaphore-based rate limiting (max 10 concurrent)
- Timeout protection: 5-minute timeout for entire batch processing
- Non-streaming API calls for batch completion (simpler and more efficient)
- Tracks performance metrics (duration per item in milliseconds)
- Returns complete JSON array response (not streaming)

## Key Implementation Details

**JSONL Upload & Parsing** (`ui.tsx:onFileDropped`):
...
- Process items in parallel using `asyncio.gather()` with semaphore rate limiting (max 10 concurrent)
- Use non-streaming API calls (more efficient for batch completion)
- Timeout protection: 5-minute timeout for entire batch, returns 504 on timeout
...
- SWR mutation manages loading state and error handling
...
- **Clear loading state**: Single spinner (managed by SWR) instead of progressive updates
- **Better for downloads**: All results ready before export
- **Easier pagination**: No partial results complicating state management
- **Rate limiting**: Semaphore-based concurrency control prevents API limits (max 10 concurrent)
...
- File upload with validation: `onFileDropped()` callback (lines 94-147 in ui.tsx)
- SWR mutation: `useSWRMutation` hook usage (lines 129-134 in ui.tsx)
- Batch classification: `onClassifyClicked()` function (lines 208-267 in ui.tsx)
- Result export: `exportResults()` function (lines 236-250 in ui.tsx)
